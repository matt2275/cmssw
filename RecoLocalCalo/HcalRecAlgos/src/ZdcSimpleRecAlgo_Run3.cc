...
void ZdcSimpleRecAlgo_Run3::initTemplateFit(const std::vector<unsigned int>& bxTs, const std::vector<double>& chargeRatios, const int nTs,const int ZdcSection){
     ...
     Eigen::MatrixXf a(nTs,nCols);
      for( int j = 0; j < nBx; j++){
        ...
        for( int i =0 ; i < nTs; i++){
           val = 0;
           index = i - timeslice;        
           if(index >= 0 && index < nRatios) val = chargeRatios.at(index);
           a(i,j) = val;
        }
     }
     for( int i =0 ; i < nTs; i++)a(i,nBx) = 1;   
     Eigen::MatrixXf b = a.transpose()*a;
     if(std::fabs(b.determinant()) < 1E-8){ templateFitValid_[ZdcSection] = false; return;}
     templateFitValid_[ZdcSection]= true;  
     Eigen::MatrixXf tfMatrix;
     tfMatrix =b.inverse()*a.transpose();
     for( int i =0 ; i < nTs; i++) templateFitValues_[ZdcSection].push_back( tfMatrix.coeff(1,i));  
...
namespace zdchelper {
  ...
  inline double getNoiseOOTPURatio(const float energy0, const float energy1, const float ootpuRatio, const float ootpuFrac){
    if( energy0 >= ootpuRatio*energy1 || ootpuRatio < 0) return( ootpuFrac*energy1);
    else return(energy1);
  }
...
ZDCRecHit ZdcSimpleRecAlgo_Run3::reconstruct(const QIE10DataFrame& digi,
      ...
      return ZdcSimpleRecAlgo_Run3::reco0(
